...
async function buatJurnalOtomatis(keterangan, nominal, tanggal) {
    ...
    else if (ket === 'belum membayar gaji') {
        entries.push(
            {
                id: Date.now().toString() + '-1',
                tipe_transaksi: 'Jurnal',
            ...
            },
            {
                ...
                kategori: 'Jurnal Otomatis',
                nominal: nominal,
                keterangan: 'Utang Gaji (K)',
                tanggal: tanggal,
                created_at: new Date().toISOString(),
                akun_target: 'Utang Gaji',
                posisi_jurnal: 'Kredit',
                is_jurnal_otomatis: true
            }
        );
        ...
        entries.push(
            {
                ...
                kategori: 'Jurnal Otomatis',
                nominal: nominal,
                keterangan: 'Beban Reparasi (D)',
                tanggal: tanggal,
                created_at: new Date().toISOString(),
                akun_target: 'Beban Reparasi',
                posisi_jurnal: 'Debet',
                is_jurnal_otomatis: true
            },
            {
...
console.log('✅ Jurnal entries created:', entries.length);
    return entries;
}
...
function renderRiwayat() {
    ...
    if (!riwayatList) {
        console.error('Element riwayat-list not found');
        return;
    }
    
    console.log('🔄 Rendering Riwayat with', transaksiList.length, 'total transactions');
    
    // Filter hanya transaksi dari kasir (bukan jurnal otomatis)
    const transaksiKasir = transaksiList.filter(t => !t.is_jurnal_otomatis);
    
    console.log('📊 Transaksi Kasir:', transaksiKasir.length);
    ...
    console.log('✅ Riwayat rendered successfully');
}

// ========== FUNGSI RENDER JURNAL UMUM ==========
function renderJurnalUmum() {
    const jurnalBody = document.getElementById('jurnal-umum-body');
    
    if (!jurnalBody) {
        console.error('Element jurnal-umum-body not found');
        return;
    }
    
    console.log('🔄 Rendering Jurnal Umum');
    
    const jurnalEntries = transaksiList.filter(t => t.tipe_transaksi === 'Jurnal');
    
    console.log('📊 Jurnal Entries:', jurnalEntries.length);
    
    if (jurnalEntries.length === 0) {
        jurnalBody.innerHTML = '<tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">Belum ada data jurnal</td></tr>';
        return;
    }
    ...
    console.log('✅ Jurnal Umum rendered successfully');
}

// ========== FUNGSI RENDER BUKU BESAR ==========
function renderBukuBesar() {
    ...
    if (!container || !pilihAkun) {
        console.error('Buku Besar elements not found');
        return;
    }
    
    console.log('🔄 Rendering Buku Besar');
    ...
    console.log('✅ Buku Besar rendered successfully');
}

// ========== FUNGSI RENDER NERACA SALDO ==========
function renderNeracaSaldo() {
    const tbody = document.getElementById('neraca-saldo-body');
    
    if (!tbody) {
        console.error('Element neraca-saldo-body not found');
        return;
    }
    
    console.log('🔄 Rendering Neraca Saldo');
    ...
    const kategoriAkun = {
        ...
        'BEBAN': ['Harga Pokok Penjualan', 'Beban Gaji dan Upah', 'Beban Listrik dan Air', 'Beban Reparasi', 
                  'Beban Perlengkapan', 'Beban Sewa', 'Beban Lain-Lain', 'Beban Penyusutan Peralatan', 'Beban Penyusutan Bangunan']
    };
    ...
    console.log('✅ Neraca Saldo rendered successfully');
}
// ========== FUNGSI RENDER LAPORAN KEUANGAN ==========
function renderLaporanKeuangan() {
    const container = document.getElementById('laporan-keuangan-container');
    
    if (!container) {
        console.error('Element laporan-keuangan-container not found');
        return;
    }
    
    console.log('🔄 Rendering Laporan Keuangan');
    ...
    const penyusutanPeralatan = Math.abs(saldoMap['Beban Penyusutan Peralatan'] || 0);
    const penyusutanBangunan = Math.abs(saldoMap['Beban Penyusutan Bangunan'] || 0);
    ...
    const kasTangan = (saldoAwal['Kas di Tangan']?.nominal || 0) + (saldoMap['Kas di Tangan'] || 0);
    const kasBank = (saldoAwal['Kas di Bank']?.nominal || 0) + (saldoMap['Kas di Bank'] || 0);
    const piutang = saldoMap['Piutang Usaha'] || 0;
    const persediaan = (saldoAwal['Persediaan Barang']?.nominal || 0) + (saldoMap['Persediaan Barang'] || 0);
    const sewaDibayarDimuka = saldoMap['Sewa Dibayar di Muka'] || 0;
    const perlengkapan = (saldoAwal['Perlengkapan']?.nominal || 0) + (saldoMap['Perlengkapan'] || 0);
    
    const peralatan = (saldoAwal['Peralatan']?.nominal || 0) + (saldoMap['Peralatan'] || 0);
    const akumPenyPeralatan = (saldoAwal['Akumulasi Penyusutan Peralatan']?.nominal || 0) + Math.abs(saldoMap['Akumulasi Penyusutan Peralatan'] || 0);
    const bangunan = (saldoAwal['Bangunan']?.nominal || 0) + (saldoMap['Bangunan'] || 0);
    const akumPenyBangunan = (saldoAwal['Akumulasi Penyusutan Bangunan']?.nominal || 0) + Math.abs(saldoMap['Akumulasi Penyusutan Bangunan'] || 0);
    ...
    const hutangJangkaPanjang = saldoAwal['Hutang Usaha Jangka Panjang']?.nominal || 0 + Math.abs(saldoMap['Hutang Usaha Jangka Panjang'] || 0);
    
    const totalLiabilitas = hutangUsaha + utangGaji + hutangListrik + hutangJangkaPanjang;
    
    const modal = saldoAwal['Modal']?.nominal || 0 + Math.abs(saldoMap['Modal'] || 0);
    const prive = saldoAwal['Prive']?.nominal || 0 + Math.abs(saldoMap['Prive'] || 0);
    ...
    console.log('✅ Laporan Keuangan rendered successfully');
}
...
// ========== FUNGSI RENDER LAPORAN KAS ==========
function renderLaporanKas() {
    const container = document.getElementById('laporan-kas-container');
    
    if (!container) {
        console.error('Element laporan-kas-container not found');
        return;
    }
    
    console.log('🔄 Rendering Laporan Kas');
...
// ========== FUNGSI RENDER LAPORAN MODAL ==========
function renderLaporanModal() {
    const container = document.getElementById('laporan-modal-container');
    
    if (!container) {
        console.error('Element laporan-modal-container not found');
        return;
    }
    
    console.log('🔄 Rendering Laporan Modal');
    ...
    console.log('✅ Laporan Modal rendered successfully');
}
...
function renderDiagram() {
    ...
    if (jurnalEntries.length === 0) {
        console.log('⚠️ No jurnal entries for diagram');
        return;
    }
    ...
    setTimeout(() => {
        ...
        // Data untuk Aset
        const kasTangan = (saldoAwal['Kas di Tangan']?.nominal || 0) + Math.abs(saldoMap['Kas di Tangan'] || 0);
        const kasBank = (saldoAwal['Kas di Bank']?.nominal || 0) + Math.abs(saldoMap['Kas di Bank'] || 0);
        const piutang = Math.abs(saldoMap['Piutang Usaha'] || 0);
        const persediaan = (saldoAwal['Persediaan Barang']?.nominal || 0) + Math.abs(saldoMap['Persediaan Barang'] || 0);
        const peralatan = (saldoAwal['Peralatan']?.nominal || 0) + Math.abs(saldoMap['Peralatan'] || 0);
        const bangunan = (saldoAwal['Bangunan']?.nominal || 0) + Math.abs(saldoMap['Bangunan'] || 0);
        // Chart Aset (Pie)
    ...
    }, 500); // Delay 500ms untuk memastikan Chart.js ready
...
function arsipkanHalaman(name) { 
    showToast(name + ' - arsip belum tersedia di mode terpisah', 'error'); 
}
...
async function resetData() { 
    if (confirm('Apakah Anda yakin ingin mereset semua data? Data akan hilang permanen!')) {
        localStorage.clear();
        transaksiList = [];
        window.menuInfoMap = {};
        location.reload();
...
function exportData() {
    ...
    a.href = url;
    a.download = "kings_steak_backup_" + new Date().toISOString().split('T')[0] + ".json";
    a.click();
    URL.revokeObjectURL(url);
    showToast("Export berhasil! File JSON sudah diunduh.", 'success');
}
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            showToast("Import data berhasil! Refresh halaman.", 'success');
            setTimeout(() => location.reload(), 1500);
        } catch (error) {
            showToast("Gagal membaca file. Pastikan file JSON benar.", 'error');
        }
    };
...
async function init() {
    ...
    try {
        ...
        if (!savedTransaksi || savedTransaksi === '[]' || savedTransaksi === 'null') {
            console.log('📦 Tidak ada data, memuat data awal...');
            
            // Cek apakah initialData tersedia
            if (typeof initialTransaksi !== 'undefined' && Array.isArray(initialTransaksi)) {
                transaksiList = [...initialTransaksi];
                window.menuInfoMap = typeof initialMenuInfo !== 'undefined' ? {...initialMenuInfo} : {};
                
                // Generate jurnal untuk data awal
                console.log('📝 Generating jurnal entries...');
                ...
                console.warn('⚠️ initialData.js tidak ditemukan, menggunakan array kosong');
                transaksiList = [];
                window.menuInfoMap = {};
            }
        } else {
...
function downloadPDFWithHeader(elementId, filename, title) {
    ...
    // Clone element untuk diproses
    const clonedElement = element.cloneNode(true);
    
    // Format semua angka di dalam clone
    const allTextNodes = [];
    ...
    // Format angka yang belum terformat
    allTextNodes.forEach(textNode => {
        let text = textNode.textContent;
        
        // Cari pola angka besar (lebih dari 1000)
    ...
    const printContent = `
        ...
        <body>
            <div class="header">
                <h1>${title}</h1>
                <p><strong>Kings Steak</strong></p>
                <p>Jl. Taman Siswa, Gunung Pati</p>
                <p><strong>${getPeriodeString()}</strong></p>
                <p>Dicetak: ${new Date().toLocaleString('id-ID')}</p>
            </div>
